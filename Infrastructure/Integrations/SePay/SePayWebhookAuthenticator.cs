...
public sealed class SePayWebhookAuthenticator(IOptions<SePayWebhookOptions> options)
{
    ...
    public SePayAuthenticationResult Authenticate(IHeaderDictionary headers, string rawBody)
    {
        return _options.AuthenticationMode.Trim().ToLowerInvariant() switch
        {
            "none" => SePayAuthenticationResult.Success,
            "apikey" => AuthenticateApiKey(headers),
            "hmacsha256" or "hmac-sha256" => AuthenticateHmac(headers, rawBody),
    ...
    private SePayAuthenticationResult AuthenticateHmac(IHeaderDictionary headers, string rawBody)
    {
        ...
        if (Math.Abs(now - timestamp) > _options.AllowedClockSkewSeconds)
